...
function HomePage() {
  ...
  const [refreshKey, setRefreshKey] = useState(0); // для перезагрузки каталога после импорта
  ...
  const importFilm = useCallback(async (tmdbId, filmTitle) => {
    ...
    try {
      ...
      setRefreshKey(prev => prev + 1); // Триггерим перезагрузку каталога
  ...
  return (
    <div className="container">
      ...
      <div className="catalog-section">
        ...
        <FilmsCatalog key={refreshKey} initialSort="technical" limit={20} showSortTabs={true} />
      </div>
    </div>
  );
}
