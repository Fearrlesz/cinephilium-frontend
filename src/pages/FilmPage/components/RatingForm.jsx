...
function RatingForm({
  scores = {}, // 👈 Добавляем дефолтные значения
  vibe = 5,
  genrePreset = '',
  ...
  const getScore = (blockKey, critKey) => {
    return scores?.[blockKey]?.[critKey] ?? 5; // fallback 5
  };

  // Мемоизируем preview, чтобы не пересчитывать при каждом рендере
  ...
  return (
    <>
        ...
        return (
          <div key={block.key} className="criteria-block">
            ...
            {block.criteria.map(crit => {
              const value = getScore(block.key, crit.key); // 👈 БЕЗОПАСНО!
              return (
      ...
      {/* 🆕 ДОБАВЛЯЕМ ПОЛЕ ДЛЯ ТЕКСТОВОГО ОТЗЫВА */}
